  ...
  setup() {
    ...
    const registerExit = async () => {
      ...
      if (user) {
        ...
        const registroSnapshot = await getDocs(registrosRef);
        const registroHoy = registroSnapshot.docs.find(
          (doc) => doc.data().fecha === today && !doc.data().horaSalida
        );

        if (registroHoy) {
          // Si existe un registro para el día actual sin hora de salida, añade la hora de salida
          const registroRef = doc(registrosRef, registroHoy.id);
          await updateDoc(registroRef, {
            horaSalida: now.toLocaleTimeString(),
          });
        } else {
          // Si no existe un registro para el día actual sin hora de salida, crea uno nuevo
          await addDoc(registrosRef, {
            fecha: today,
            horaSalida: now.toLocaleTimeString(),
          });
